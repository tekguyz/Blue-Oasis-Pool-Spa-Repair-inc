      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row items-center justify-between gap-10">
          ...
          <div className="flex items-center gap-6 flex-wrap justify-center lg:justify-start">
            <div className="flex text-warm-orange" aria-label="5 out of 5 stars">
              {[1, 2, 3, 4, 5].map((i) => (
            ...
            <div className="flex items-center gap-6 border-l border-gray-200 pl-8 flex-wrap justify-center">
              {reviewPlatforms.map((platform) => (
                <span key={platform} className="font-heading font-black text-deep-navy/90 text-xl tracking-tight">
                  {platform}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-center gap-10 opacity-40 grayscale flex-wrap justify-center lg:justify-end">
            <span className="text-xs font-black uppercase tracking-[0.2em] text-deep-navy/60 hidden xl:block mr-2">Authorized Brands:</span>
            {brands.map((brand) => (
              <span key={brand} className="font-heading font-black text-2xl tracking-tighter">
                {brand}
              </span>
            ))}
          </div>
